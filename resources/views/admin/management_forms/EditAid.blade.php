@extends('layouts.app')
@section('css')
    <title>{{ config('app.name') }} Edit Aid</title>
    <!-- Default-->
...
@section('content')
<!-- Main content -->
<form class="form-horizontal my-4" action="/EditAid/{{ $Receiving_aid->date }}/{{ $Receiving_aid->number }}" method="POST" autocomplete="on">
  @csrf
    <div class="row">
        <div class="col-xl-12">
          <div class="card">
                ...
                <div class="row form-material">
                    <div class="col-md-6">
                        <label for="date" class="col-form-label text-right"> التاريخ  </label>
                            <input type="date" class="form-control @error('date') is-invalid @enderror" id="date" name="date" value="{{ $Receiving_aid->date }}" autocomplete="date"  autofocus required="">
                            @error('date')
                    ...
                    <div class="col-md-6">
                        <label for="employee_name" class="col-form-label text-right"> اسم أمين المستودع  </label>
                            <input type="text" class="form-control @error('employee_name') is-invalid @enderror mx" id="employee_name" name="employee_name" value="{{ $Receiving_aid->employee_name }}" autocomplete="employee_name" placeholder="~" maxlength="100" autofocus required="">
                            @error('employee_name')
                              <span class="invalid-feedback" role="alert">
                                <strong>{{ $message }}</strong>
                              </span>
                ...
                <div class="row form-material">
                    <div class="col-md-12">
                        <div class="repeater">
                            ...
                            @foreach ($Receiving_aid->receiving_aid_details as $item_aid)
                                <div data-repeater-item class="row">
                                    <div class="col-md-6">
                                        <label for="item" class="col-form-label text-right"> المواد المسلمة  </label>
                                            <input type="text" class="form-control @error('item') is-invalid @enderror mx" id="item" name="item" value="{{ $item_aid->item }}" autocomplete="item" placeholder="~" maxlength="100" autofocus required="">
                                            @error('item')
                                              <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                              </span>
                                    ...
                                    <div class="col-md-3">
                                      <label for="measuring_unit_id" class="col-form-label text-right"> الوحدة  </label>
                                        <select class="form-control mb-3 @error('measuring_unit_id') is-invalid @enderror" style="width: 100%; height:36px;" id="measuring_unit_id" name="measuring_unit_id" autocomplete="measuring_unit_id" autofocus required >
                                            <option selected="selected" value="{{$item_aid->measuring_unit->id}}">{{$item_aid->measuring_unit->name}}</option>
                                    ...
                                    <div class="col-md-2">
                                        <label for="quantity" class="col-form-label text-right"> الكمية  </label>
                                            <input type="number" class="form-control @error('quantity') is-invalid @enderror" id="quantity" name="quantity" value="{{ $item_aid->quantity }}" autocomplete="quantity" placeholder="~" autofocus required="">
                                            @error('quantity')
                                              <span class="invalid-feedback" role="alert">
                                                <strong>{{ $message }}</strong>
                                              </span>
                            ...
                            @endforeach
                                ...
                                <input type="hidden" id="count_item" name="count_item" value="{{ $Receiving_aid->receiving_aid_details->count() }}">
                            </div>
