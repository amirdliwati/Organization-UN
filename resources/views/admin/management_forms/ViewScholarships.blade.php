@extends('layouts.app')
@section('css')
<title>{{ config('app.name') }} Export Scholarships</title>
    <!-- Default-->
...
@section('content')
<!-- Main content -->
<form class="form-horizontal my-4" action="/ExportScholarships" method="POST" autocomplete="on">
  @csrf
  <div class="row">
      <div class="col-xl-12">
        <div class="card">
          <h5 class="card-header bg-success text-white mt-0"><i class="mdi mdi mdi-file-document-box-search-outline mr-1"></i>  بحث في المنح  </h5>
          <div class="card-body">
            <div class="row form-material">
              <div class="col-md-3">
                <label for="Receiving_scholarship_id" class="col-form-label text-right"> تاريخ و رقم المنحة  </label>
                  <select class="select2 form-control mb-3 @error('Receiving_scholarship_id') is-invalid @enderror" style="width: 100%; height:36px;" id="Receiving_scholarship_id" name="Receiving_scholarship_id" autocomplete="Receiving_scholarship_id" autofocus required >
                      @foreach($Receiving_scholarships as $Receiving_scholarship)
                      <option value="{{$Receiving_scholarship->id}}">{{$Receiving_scholarship->date}}{{_(' -> ')}}{{$Receiving_scholarship->number}}</option>
                      @endforeach
                  </select>
                  @error('Receiving_scholarship_id')
  ...
  <div class="row">
      <div class="col-xl-12">
        <div class="card">
          <h5 class="card-header bg-info text-white mt-0"><i class="mdi mdi-file-document-outline mr-1"></i>  تصدير المنح  </h5>
          <div class="card-body">
