@extends('layouts.app')
@section('css')
<title>{{ config('app.name') }} View Scholarship</title>
    <!-- Default-->
                ...
                <tbody>
                  @foreach ($Scholarships as $Scholarship)
                    <tr>
                        ...
                        <td>{{$Scholarship->serial_number}}</td>
                        <td>{{$Scholarship->number}}</td>
                        <td>{{$Scholarship->employee_name}}</td>
                        <td><button type="button" class="btn btn-danger btn-rounded waves-effect waves-light" onclick="Delete('{{$Scholarship->serial_number}}')"><i class="mdi dripicons-trash mr-2"></i>  حذف  </button></td>
                    </tr>
                  @endforeach
                    ...
                    cancelButtonText: '<i class="fa fa-thumbs-down"></i> لا',
                        ...
                        xmlhttp.onreadystatechange = function() {
                            if (this.readyState == 4 && this.status == 200) {
                                swal({title: 'نجاح العملية !',text: 'تم حذف  المنحة  بنجاح  ',type: 'success',
                        ...
                        xmlhttp.open("GET", "{{url('delete-scholarship')}}?id="+id, true);
                        xmlhttp.send();
                    }
                })
            };
